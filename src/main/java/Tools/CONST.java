package Tools;

public class CONST {

    /// <summary>
    /// Applications Source Environments
    /// </summary>
    public static String SELENIUM_HUB = "http://34.94.107.122:4444/wd/hub";
    public static String BASE_URL = "https://comfy.ua/gaming";
    public static String LOGIN_URL = "https://comfy.ua/gaming/";
    public static String LOGOUT_URL = "https://comfy.ua/gaming/";
    public static String FIREFOX_BROWSER = "Firefox";
    public static String CHROME_BROWSER = "Chrome";
    public static String REMOTE_BROWSER = "RemoteBrowser";
    public static String PATH_LINUX_CHROME_DRIVER = "/usr/bin/chromedriver";
    public static String PATH_WINDOWS_CHROME_DRIVER = "D:\\Chrome\\chromedriver_win32\\chromedriver.exe";
}
